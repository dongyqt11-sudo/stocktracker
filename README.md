...
- 自选列表：`GET /api/watchlist?account_id=`
- 添加自选：`POST /api/watchlist`
...
本地 OCR 偶尔会把 ETF 现价的小数点漏掉，例如把 `1.998` 识别成 `998`。系统在上传识别和确认入库时会自动校验：

```text
现价 ≈ 市值 ÷ 持仓数量
```

如果偏差明显超过合理范围，会自动用“市值 ÷ 持仓数量”反推现价，并在原始 JSON 中保留修正痕迹。已经入库的历史数据可以运行：

```powershell
python scripts/fix_holding_prices.py
```
